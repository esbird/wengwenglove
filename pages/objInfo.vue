...
export default {
  ...
  async asyncData({query}) {
    ...
    //     return res;
    //   })
    await getMyInfo({uid:query.Oid}).then(res=>{
        if(res.data.code==200){
            ayData.objInfo = res.data.data.userinfo;
        }
    })
    // 获取推荐人的照片
    ayData.objInfo.userid = ayData.objInfo.id;
    await getPhotsByUid({ uid: ayData.objInfo.id })
      .then(res=>{
        // console.log(res)
        if(res.data.code==200){
